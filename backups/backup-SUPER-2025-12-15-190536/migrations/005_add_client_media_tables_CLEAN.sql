-- ============================================================================
-- MIGRATION 005: Client Media & Send Rules (CLEAN VERSION)
-- ============================================================================
-- Versão LIMPA que dropa e recria tudo

-- Drop índices existentes (se houver)
DROP INDEX IF EXISTS idx_client_media_client;
DROP INDEX IF EXISTS idx_client_media_agent;
DROP INDEX IF EXISTS idx_client_media_tags;
DROP INDEX IF EXISTS idx_client_media_type;
DROP INDEX IF EXISTS idx_client_media_category;
DROP INDEX IF EXISTS idx_client_media_active;

DROP INDEX IF EXISTS idx_media_rules_client;
DROP INDEX IF EXISTS idx_media_rules_agent;
DROP INDEX IF EXISTS idx_media_rules_type;
DROP INDEX IF EXISTS idx_media_rules_keywords;
DROP INDEX IF EXISTS idx_media_rules_active;

DROP INDEX IF EXISTS idx_media_log_conversation;
DROP INDEX IF EXISTS idx_media_log_client;
DROP INDEX IF EXISTS idx_media_log_rule;

-- Drop functions existentes
DROP FUNCTION IF EXISTS search_client_media(TEXT, TEXT, TEXT[], TEXT, TEXT);

-- Drop tabelas existentes (em ordem reversa de dependências)
DROP TABLE IF EXISTS media_send_log;
DROP TABLE IF EXISTS media_send_rules;
...
VALUES 
  ...
  (
    ...
    ARRAY['servicos', 'precos', 'tratamentos', 'valores'],
    'services'
  );
...
VALUES
  (
    ...
    ARRAY['consultório', 'consultorio', 'ambiente', 'onde fica', 'local'],
  ...
  (
    ...
    ARRAY['equipe', 'time', 'dentistas', 'profissionais', 'quem atende'],
  ...
  (
    ...
    'default',
    'keyword_trigger',
    'Enviar tabela de preços quando perguntar',
    ARRAY['preço', 'preco', 'valor', 'quanto custa', 'tabela', 'valores'],
    (SELECT id FROM client_media WHERE file_name = 'tabela-precos.pdf' LIMIT 1),
    3
  );
